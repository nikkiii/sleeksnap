...
/**
 * An uploader to save images and text to local files (Working
 * Directory/local/<images/text>)
 * 
 * @author Nikki
 * 
 */
public class LocalFileUploader extends GenericUploader {
	...
	/**
	 * An ImageUploader using ImageIO.write(image, format, file) to save an
	 * image
	 * 
	 * @author Nikki
	 * 
	...
	public class ImageLocalFileUploader extends Uploader<ImageUpload> {
		...
		@Override
		public String upload(final ImageUpload upload) throws Exception {
			checkDirectory(upload);
			final File file = new File(LOCAL_IMAGE_DIR,
					FileUtils.generateFileName(upload));
	...
	public class TextLocalFileUploader extends Uploader<TextUpload> {
		...
		@Override
		public String upload(final TextUpload upload) throws Exception {
			checkDirectory(upload);
			final File file = new File(LOCAL_TEXT_DIR,
					FileUtils.generateFileName(upload));
	 ...
	 */
	private static final File LOCAL_BASE_DIR = new File(
			Util.getWorkingDirectory(), "local");

	/**
	 * Local image directory
	 */
	private static final File LOCAL_IMAGE_DIR = new File(LOCAL_BASE_DIR,
			"images");
	...
	private final Uploader<?>[] uploaders = new Uploader<?>[] {
			new ImageLocalFileUploader(), new TextLocalFileUploader() };
	 ...
	 */
	protected void checkDirectory(final Upload upload) {
