...
public class UpdaterPanel extends OptionSubPanel {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -7637744647306468021L;
	...
	public UpdaterPanel(final OptionPanel optionPanel) {
	...
	public void initComponents() {
		...
		updaterMode.setModel(new javax.swing.DefaultComboBoxModel(new String[] {
				"Automatic", "Prompt", "Manual" }));

		releaseType.setModel(new javax.swing.DefaultComboBoxModel(new String[] {
				"Recommended", "Development" }));

		final UpdaterMode mode = optionPanel.getSnapper().getConfiguration()
				.getEnumValue("updateMode", UpdaterMode.class);
		if (mode != null) {
			updaterMode.setSelectedIndex(mode.ordinal());
		}

		final UpdaterReleaseType type = optionPanel.getSnapper()
				.getConfiguration()
				.getEnumValue("updateReleaseType", UpdaterReleaseType.class);
		if (mode != null) {
		...
		updaterMode.addItemListener(new ItemListener() {
			...
			public void itemStateChanged(final ItemEvent e) {
				if (e.getStateChange() == ItemEvent.SELECTED) {
					// Set config to this.
					optionPanel.getSnapper().getConfiguration()
							.put("updateMode", updaterMode.getSelectedIndex());
					try {
			...
			public void itemStateChanged(final ItemEvent e) {
				if (e.getStateChange() == ItemEvent.SELECTED) {
					...
					optionPanel
							.getSnapper()
							.getConfiguration()
							.put("updateReleaseType",
									releaseType.getSelectedIndex());
					try {
						optionPanel.getSnapper().getConfiguration().save();
					} catch (final IOException e1) {
						e1.printStackTrace();
		...
		checkButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(final ActionEvent e) {
				...
				UpdaterReleaseType type = optionPanel
						.getSnapper()
						.getConfiguration()
						.getEnumValue("updateReleaseType",
								UpdaterReleaseType.class);
				if (type == null) {
					type = UpdaterReleaseType.RECOMMENDED;
				}

				if (!updater.checkUpdate(type, true)) {
					JOptionPane.showMessageDialog(UpdaterPanel.this,
							"No new updates available.", "Sleeksnap Update",
		...
		final javax.swing.GroupLayout updaterPanelLayout = new javax.swing.GroupLayout(
				this);
		setLayout(updaterPanelLayout);
		updaterPanelLayout
				.setHorizontalGroup(updaterPanelLayout
						.createParallelGroup(
														...
														.addGroup(
																updaterPanelLayout
																		...
																		.addGap(316,
																				...
																				316)))));
		updaterPanelLayout
				.setVerticalGroup(updaterPanelLayout
						.createParallelGroup(
								javax.swing.GroupLayout.Alignment.LEADING)
						.addGroup(
								updaterPanelLayout
